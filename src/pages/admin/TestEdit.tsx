...
export default function TestEdit() {
  ...
  useEffect(() => {
    const fetchTest = async () => {
      ...
      try {
        alert("sasa"+testId)
        setLoading(true)
        const response = await api.get(`/api/admin/tests/${testId}`)
        const test = response.data

        // Format dates for datetime-local input
        const formattedStartTime = new Date(test.startTime).toISOString().slice(0, 16)
        const formattedEndTime = new Date(test.endTime).toISOString().slice(0, 16)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Send the request
      await api.put(`/api/admin/tests/${testId}`, testData)

      alert("Test muvaffaqiyatli yangilandi!")
      navigate("/admin/tests")
    } catch (error) {
      console.error("Error updating test:", error)
      setError("Testni yangilashda xatolik yuz berdi.")
    } finally {
      setSaving(false)
    }
  }
  ...
  return (
    <div className="pt-4 pb-24">
      <div className="flex justify-between items-center mb-4 px-4">
        <h1 className="text-2xl font-bold text-gray-800">Testni tahrirlash</h1>
      </div>
